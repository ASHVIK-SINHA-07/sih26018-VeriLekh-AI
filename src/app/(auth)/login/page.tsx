              ...
              <tbody>
                {DEMO_ACCOUNTS.map((account) => (
                  <tr key={account.email} className="align-baseline">
                    ...
                    <td className="py-0.5 font-mono text-[11.5px] text-ink-2">{account.password}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            <p className="mt-2 text-[11.5px] leading-snug text-muted-foreground">{t("login.demoNote")}</p>
          </div>
        ) : null}

        <div className="flex flex-wrap items-center justify-between gap-3">
          <Link href="/check" className="text-[13px] text-navy underline underline-offset-2">
            {t("login.checkRecord")}
          </Link>
          <div className="w-52">
